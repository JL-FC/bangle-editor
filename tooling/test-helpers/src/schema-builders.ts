...
const isNotValidLabel = (label: keyof typeof labelTypes) =>
  !Boolean(labelTypes[label]);

const updateMap = (map: WeakMap<any, any>, key: any, value: any) =>
  map.set(key, safeMergeObject(map.get(key), value));

export const getDocLabels = weakCache((doc: Node) => {
  ...
  doc.nodesBetween(0, doc.content.size, (node, pos) => {
    ...
    const resolvedLabels = objectMapValues(
      found,
      (v: number | ((pos: number, cont: any) => number)) => {
        return typeof v === 'function'
          ? v(pos, (node.content as any).content)
          : pos + v;
      },
    );
 ...
 */
export function psx(
  name: keyof typeof nodeAlias | keyof typeof markAlias,
  attrs: any,
  ...childrenRaw: any[]
) {
  return (schema: Schema) => {
    const children = []
      .concat(...childrenRaw)
      .map((c: any) => (typeof c === 'string' ? c : c(schema)));
    ...
    const markSpec =
      schema.marks[name] || schema.marks[(markAlias as any)[name]];
